#!/usr/bin/env bash
#SBATCH --job-name=tier3c-composition
...
#SBATCH --export=NONE
#SBATCH --output=analysis/pilot_results/logs/tier3c-%A_%a.out
#SBATCH --error=analysis/pilot_results/logs/tier3c-%A_%a.err
set -euo pipefail
...
exec "$TIER3_ROOT/analysis/slurm/guix_job.sh" "$ENVIRONMENT_RECORD" \
    ...
    --tier 3c --index "${SLURM_ARRAY_TASK_ID:?array index missing}"
