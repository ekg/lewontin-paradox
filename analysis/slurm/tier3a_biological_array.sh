#!/usr/bin/env bash
#SBATCH --job-name=tier3a-diploid
#SBATCH --array=0-2
...
#SBATCH --export=NONE
#SBATCH --output=results/tier3a/logs/tier3a-%A_%a.out
#SBATCH --error=results/tier3a/logs/tier3a-%A_%a.err
...
row = rows[index]
for key in ("dataset_id", "sweepga_bounded_paf_path", "h1_fasta_path", "h2_fasta_path", "impg_binary_path"):
    print(row[key])
PY
)
[[ ${#META[@]} -eq 5 ]] || { echo "Incomplete acquisition metadata at array index $INDEX" >&2; exit 2; }
DATASET=${META[0]}
PAF=${META[1]}
H1=${META[2]}
H2=${META[3]}
...
mkdir -p "$OUT" "$OUT/partitions" "$OUT/calls"
...
START_EPOCH=$(date +%s)
if [[ ! -s $OUT/graph.impg || ! -s $OUT/partitions/partitions.bed ]]; then
