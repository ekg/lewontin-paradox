...
# tier3a remap syntax (--num-mappings 1:1, wfmash aligner backend, recorded
...
printf '%q ' "$sweepga" "$scratch/inputs/h2.fa" "$scratch/inputs/h1.fa" \
    --output-file "$native_paf" --aligner wfmash --map-pct-identity 90 \
    --min-aln-length 25k --num-mappings 1:1 --scaffold-jump 0 \
    --overlap 0.95 --scoring log-length-ani --threads "$threads" \
    --temp-dir "$scratch/wfmash-temp" > "$partial/command.txt"
printf '\n' >> "$partial/command.txt"
...
mkdir -p "$scratch/wfmash-temp"

"$sweepga" "$scratch/inputs/h2.fa" "$scratch/inputs/h1.fa" \
    --output-file "$native_paf" --aligner wfmash --map-pct-identity 90 \
    --min-aln-length 25k --num-mappings 1:1 --scaffold-jump 0 \
    --overlap 0.95 --scoring log-length-ani --threads "$threads" \
    --temp-dir "$scratch/wfmash-temp" \
...
Path(sys.argv[1]).write_text(json.dumps({
    ...
    "aligner_provenance": "sweepga origin/main pinned build (tier3a remap template)",
