...
# .complete.json) using the pinned origin/main SweepGA binary.
...
# Parameterized via VGP_SELECTION / VGP_INPUT_DIR / VGP_RUN_ROOT / VGP_BUILD_JSON
# so the same script serves P09 and other wave pairs (e.g. P08/menidia).
...
stage "launch sweepga/fastga (pid written when backgrounded)"
...
Path(sys.argv[1]).write_text(json.dumps({
    ...
    "mapping_backend": "fastga",
    "aligner_provenance": "sweepga origin/main pinned build; fastga backend (sweepga default), accepted-core flag semantics (P02 job 1782140)",
