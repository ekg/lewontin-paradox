...
INPUT_MANIFESTS = {
    selection_id: Path(
        f"/moosefs/erikg/vgp/pilot/inputs/{selection_id}/input-manifest.json"
    )
        ...
        with gzip.open(path, "rt") as handle:
            for number, line in enumerate(handle, 1):
                ...
                if (
                    ...
                    or set((fields[3] + fields[4]).upper()) - set("ACGTN")
                ):
                    raise PilotError(
                        f"{path}:{number}: non-sequence or monomorphic normalized allele"
                    )
...
def _graph_lengths(
    ...
    manifest = json.loads(INPUT_MANIFESTS[selection_id].read_text())
    assets = manifest["assets"]
    return (
        {
            str(row["name"]): int(row["length"])
            for row in assets["h1_fasta"]["sequence_dictionary"]
        },
        {
            str(row["name"]): int(row["length"])
            for row in assets["h2_fasta"]["sequence_dictionary"]
        },
    )
...
def audit_graph_ids_independent(
    ...
    if selection_id != "P07":
        with tempfile.TemporaryDirectory(prefix=f"vgp-{selection_id}-graph-audit-") as tmp:
            ...
            dictionary = Path(tmp) / "immutable_input_dictionary.json"
            dictionary.write_text(canonical_json({"roles": {
                "h1_fasta": {
                    "records": [
                        {"name": name, "length": length}
                        for name, length in h1.items()
                    ]
                },
                "h2_fasta": {
                    "records": [
                        {"name": name, "length": length}
                        for name, length in h2.items()
                    ]
                },
            }}))
            rebuilt = audit_graph_identifiers(
                dictionary, paf, partitions, output
            )
        for key in (
...
def audit_frozen_sources(selection: Mapping[str, object]) -> dict[str, object]:
    selection_id = str(selection["selection_id"])
    if selection_id == "P07":
        return {
            "source_identity": "closed clean-canary BGZF/CAS evidence",
            "source_objects_reverified_by_clean_canary": True,
        }
    manifest = INPUT_MANIFESTS[selection_id]
    if sha256_file(manifest) != selection["input_manifest_sha256"]:
        raise PilotError(f"{selection_id}: immutable input manifest digest changed")
    values = {}
    for role in ("h1", "h2"):
        path = Path(str(selection[f"{role}_fasta_path"]))
        observed = sha256_file(path)
        if observed != selection[f"{role}_fasta_sha256"]:
            raise PilotError(f"{selection_id}: frozen {role.upper()} FASTA digest changed")
        values[f"{role}_fasta"] = {
            "path": str(path), "sha256": observed, "size_bytes": path.stat().st_size
        }
    return {
        "input_manifest_sha256": sha256_file(manifest),
        "whole_fasta_digests_recomputed": True,
        **values,
    }
...
def audit_pair(
    ...
    return {
        ...
        "frozen_source_audit": audit_frozen_sources(selection),
