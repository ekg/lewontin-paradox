...
from analysis.materialize_vgp_bounded_evidence import (
...
def test_bounded_report_requires_and_renders_three_actual_results(tmp_path: Path):
    ...
    assert "CM078529.1:2167395" in text
    assert "zero IMPG queries" in text
    assert "technical limit" in text


def test_independent_reaudit_materializer_selects_recomputed_evidence(tmp_path: Path):
    pair = {
        "selection_id": "P07",
        **{
            key: {"passed": True}
            for key in (
                "frozen_source_audit", "closed_output_ledger",
    ...
    assert value["completion_gate_passed"] is True
    assert value["pairs"][0]["independent_mask_accounting"]["passed"] is True
