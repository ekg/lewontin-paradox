...
if [[ -n ${SLURM_JOB_ID:-}${SLURM_ARRAY_JOB_ID:-}${SLURM_ARRAY_TASK_ID:-} ]]; then
    echo "refusing: gated scale-out manifest generation is not a biological Slurm job" >&2
    exit 65
fi
...
if [[ ${1:-} == --all-tests ]]; then
    ...
    shift
    command=(python3 -m pytest -q "$ROOT/analysis/tests/test_vgp_core_scaleout.py" "$@")
else
    command=(python3 "$ROOT/analysis/vgp_core_scaleout.py" "$@")
fi
