...
The user subsequently made exactly that specification decision: literal `-n`
is non-blocking, and the unmodified native `--num-mappings 1:1` interface is
the accepted production syntax. Task `remap-tier3a-origin-sweepga` used the
byte-identical build-1 binary above for all three whole-haplotype mappings,
verified each result by a separate pinned native 1:1 recheck without replacing
the production PAF, and completed IMPG-native partition/query plus normalized,
indexed biological VCF/BCF gates. The authoritative publication artifacts are
`results/tier3a/acquisition_corrected_manifest.tsv` and
`results/tier3a/acquisition_sweepga_supersession.tsv`. The original literal
short-option observations remain in this document as historical evidence.
