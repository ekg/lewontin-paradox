...
All 3 acquired biological tuples produced coding-region estimates through pinned origin/main SweepGA native `--num-mappings 1:1` and freshly executed IMPG.
...
Native cap coverage (query,target): 1:1=(0.9305,0.9512). No higher-cap sensitivity was run for this production correction.
...
Native cap coverage (query,target): 1:1=(0.9827,0.9870). No higher-cap sensitivity was run for this production correction.
...
Native cap coverage (query,target): 1:1=(0.9491,0.9885). No higher-cap sensitivity was run for this production correction.

## Scheduler recovery

One partial attempt was discarded without artifact reuse: menidia_menidia_fMenMen1 job 1761798_0 (partial_attempt_discarded_after_live_script_parse_error_before_IMPG_query). The replacement tuple ran from an empty directory and completed successfully.
